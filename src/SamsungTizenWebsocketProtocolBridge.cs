...
namespace PepperDash.Essentials.Plugins.Samsung.TizenWebsocket.Protocol
{
	...
	public class SamsungTizenWebsocketProtocolBridge : IDisposable
	{
		...
		public async void SendKey(string cmd)
		{
			if (!IsReady)
			{
				// Deduplicate consecutive identical toggle keys (power/mute) only — repeated
				// non-toggle keys (e.g. volume ramp steps) must all be preserved and delivered.
				if (IsToggleKey(cmd))
				{
		...
		private static bool IsToggleKey(string cmd)
		{
			return cmd == SamsungTizenCommands.KeyPower || cmd == SamsungTizenCommands.KeyMute;
		}
		...
		public async Task DisconnectAsync()
		{
			try
			{
				...
				// Wake any in-progress backoff wait so its loop observes reconnectEnabled == false
				// and exits, instead of sleeping and later racing a fresh reconnect loop.
				try { reconnectDelayCts?.Cancel(); } catch { }
